...
const DEFINITIONS_DATA = [
  ...
  { param: "Lag", unit: "Blocks", source: "Diff from highest block.", relevance: "Data freshness." },
  { param: "Cost", unit: "$/mo", source: "Volume * Base Price.", relevance: "Budgeting." },
];

const TRANSPARENCY_DATA = [
    { metric: "Latency/Lag/Stability", type: "Real-Time", reason: "Measured live from your session." },
    { metric: "Cost Calculation", type: "Dynamic", reason: "Adjusts with volume slider." },
    { metric: "Capabilities/Certs", type: "Static", reason: "Hardcoded from documentation." },
];
      ...
      <div className="relative bg-[#0f172a] border border-slate-700 rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-700 bg-slate-900/50">
           <div className="flex items-center gap-3">
              ...
              <h2 className="text-lg font-bold text-white">Methodology & Data Sources</h2>
           </div>
           ...
           <div>
               ...
               <div className="rounded-xl border border-slate-700 overflow-hidden">
                   <table className="w-full text-left border-collapse">
                      <thead className="bg-slate-900/80"><tr className="text-xs uppercase text-slate-400"><th className="px-6 py-4">Param</th><th className="px-6 py-4">Source</th></tr></thead>
                      <tbody className="divide-y divide-slate-800">{DEFINITIONS_DATA.map((r,i)=><tr key={i}><td className="px-6 py-4 text-indigo-300 font-bold text-sm">{r.param}</td><td className="px-6 py-4 text-slate-400 text-sm">{r.source}</td></tr>)}</tbody>
                   </table>
               </div>
           </div>
           <div>
               ...
               <div className="rounded-xl border border-slate-700 overflow-hidden">
                   <table className="w-full text-left border-collapse">
                      <thead className="bg-slate-900/80"><tr className="text-xs uppercase text-slate-400"><th className="px-6 py-4">Metric</th><th className="px-6 py-4">Type</th><th className="px-6 py-4">Reason</th></tr></thead>
                      <tbody className="divide-y divide-slate-800">{TRANSPARENCY_DATA.map((r,i)=><tr key={i}><td className="px-6 py-4 text-slate-200 text-sm">{r.metric}</td><td className="px-6 py-4"><span className={`px-2 py-1 rounded text-[10px] uppercase font-bold border ${r.type.includes("Real")?"bg-emerald-500/10 text-emerald-400 border-emerald-500/20":r.type.includes("Static")?"bg-amber-500/10 text-amber-400 border-amber-500/20":"bg-indigo-500/10 text-indigo-400 border-indigo-500/20"}`}>{r.type}</span></td><td className="px-6 py-4 text-slate-400 text-sm">{r.reason}</td></tr>)}</tbody>
                   </table>
               </div>
           </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mb-8">
           ...
           <div className="lg:col-span-3 space-y-4">
              <GlassCard className="p-5 space-y-6 border-t-4 border-t-indigo-500/50">
                  ...
                  <div className="h-px bg-white/5"></div>
                  <button onClick={runBenchmark} disabled={isRunning} className={`w-full py-3 rounded-lg text-sm font-bold flex items-center justify-center gap-2 transition-all ${isRunning ? 'bg-slate-800 text-slate-500' : 'bg-indigo-600 hover:bg-indigo-500 text-white shadow-lg shadow-indigo-500/25'}`}>{isRunning ? <RotateCw className="w-4 h-4 animate-spin" /> : <Play className="w-4 h-4 fill-current" />}{isRunning ? 'Analyzing...' : 'Run Analysis'}</button>
              </GlassCard>
           </div>
                    ...
                    <tbody className="divide-y divide-white/5">
                      {sortedAndFilteredData.map((provider) => (
                        <tr key={provider.name} className="hover:bg-white/5 transition-colors">
                          ...
                          <td className="px-6 py-4 text-slate-300 font-mono">{provider.p99 > 0 ? <span className={provider.p99 < 300 ? 'text-indigo-400' : 'text-amber-400'}>{provider.p99} ms</span> : <span className="text-slate-600">-</span>}</td>
